...
public class Game extends AbstractGame {
    private final Image bgPicture = new Image("res/images/bg2.png");
    ...
    protected void update(Input input) {
        ...
        bgPicture.draw(Window.getWidth() / 2.0, Window.getHeight() / 2.0, loopImage);

        DrawOptions loopImage2 = new DrawOptions();
        loopImage2.setSection(0, 0, initialScrollPosition, Window.getHeight());
        bgPicture.draw(1.5*Window.getWidth() - initialScrollPosition, Window.getHeight() / 2.0, loopImage2);
        ...
        if(!isGameOver) {
            if ((x>80)&&(input.isDown(Keys.LEFT) || input.isDown(Keys.A))) {
                arrow.draw(762,700, new DrawOptions().setRotation(-3.14/2));
                xSpeed -= walkAcceleration;
            }
            if ((x<Window.getWidth()-80)&&(input.isDown(Keys.RIGHT) || input.isDown(Keys.D))) {
